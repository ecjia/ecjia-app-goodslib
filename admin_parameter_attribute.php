...
class admin_parameter_attribute extends ecjia_admin {
	public function __construct() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('商品参数模板', 'goodslib'), RC_Uri::url('goodslib/admin_parameter/init')));
	}
	...
	public function init() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('参数列表', 'goodslib')));
		$this->assign('ur_here', __('参数列表', 'goodslib'));
		...
		$this->assign('action_link', array('href' => RC_Uri::url('goodslib/admin_parameter_attribute/add', array('cat_id' => $cat_id)), 'text' => __('添加参数', 'goodslib')));
		$this->assign('action_link2', array('text' => __('参数模板列表', 'goodslib'), 'href' => RC_Uri::url('goodslib/admin_parameter/init')));
	...
	public function add() {
		...
		$cat_id = isset($_GET['cat_id']) ? intval($_GET['cat_id']) : 0;
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('参数列表', 'goodslib'), RC_Uri::url('goodslib/admin_parameter_attribute/init', 'cat_id='.$cat_id)));
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('添加参数', 'goodslib')));
		...
		$this->assign('form_action', RC_Uri::url('goodslib/admin_parameter_attribute/insert'));
	...
	public function insert() {
		...
		if (empty($cat_id)) {
			return $this->showmessage(__('请选择参数模板', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		
		$count = RC_DB::table('attribute')->where('attr_name', trim($_POST['attr_name']))->where('cat_id', $cat_id)->count();
		if ($count) {
			return $this->showmessage(__('参数名称在当前参数模板下已存在，请您换一个名称', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		if ($attr_id) {
			return $this->showmessage(sprintf(__('添加参数 [%s] 成功。', 'goodslib'), $attr['attr_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_parameter_attribute/edit', array('attr_id' => $attr_id))));
		}
	}
	...
	public function edit() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('编辑参数', 'goodslib')));
		$this->assign('ur_here', __('编辑参数', 'goodslib'));
		...
		$this->assign('form_action', RC_Uri::url('goodslib/admin_parameter_attribute/update'));
	...
	public function update() {
		...
		return $this->showmessage(sprintf(__('编辑参数 [%s] 成功。', 'goodslib'), $attr['attr_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_parameter_attribute/edit', array('attr_id' => $attr_id))));
	}
	...
	public function batch() {
		...
		if (isset($_POST['checkboxes'])) {
			...
			return $this->showmessage(sprintf(__('成功删除了 %d 条参数', 'goodslib'), $count), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_parameter_attribute/init', array('cat_id' => $cat_id))));
		}
	}
	...
	public function edit_attr_name() {
		...
		if (!empty($val)) {
			if (RC_DB::table('attribute')->where('attr_name', $val)->where('cat_id', $cat_id)->where('attr_id', '!=', $id)->count() != 0) {	
				return $this->showmessage(__('该参数名称已存在，请您换一个名称。', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
			$data = array(
				'attr_name' => $val
			);
			if (RC_DB::table('attribute')->where('attr_id', $id)->update($data)) {
				...
				return $this->showmessage(__('编辑参数名称成功', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('content' => $val));
			}
		} else {
			return $this->showmessage(__('请输入参数名称', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
	}
