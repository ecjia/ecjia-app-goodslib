...
class admin_spec_attribute extends ecjia_admin {
	...
	public function init() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('属性列表', 'goodslib')));
		$this->assign('ur_here', __('属性列表', 'goodslib'));
		...
		$this->assign('action_link', array('href' => RC_Uri::url('goodslib/admin_spec_attribute/add', array('cat_id' => $cat_id)), 'text' => __('添加属性', 'goodslib')));
		$this->assign('action_link2', array('text' => __('规格模板列表', 'goodslib'), 'href' => RC_Uri::url('goodslib/admin_spec/init')));
	...
	public function add() {
		...
		$cat_id = isset($_GET['cat_id']) ? intval($_GET['cat_id']) : 0;
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('规格属性', 'goodslib'), RC_Uri::url('goodslib/admin_spec_attribute/init', array('cat_id' => $cat_id))));
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('添加属性', 'goodslib')));

		$this->assign('ur_here', __('添加属性', 'goodslib'));
		$this->assign('action_link', array('href' => RC_Uri::url('goodslib/admin_spec_attribute/init', 'cat_id='.$cat_id), 'text' => __('属性列表', 'goodslib')));
	...
	public function insert() {
		...
		if (empty($cat_id)) {
			return $this->showmessage(__('请选择规格模板', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		
		$count = RC_DB::table('attribute')->where('attr_name', trim($_POST['attr_name']))->where('cat_id', $cat_id)->count();
		if ($count > 0) {
			return $this->showmessage(__('属性名称在当前规格模板下已存在，请您换一个名称', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		if ($attr_id) {
			...
			return $this->showmessage(sprintf(__('添加属性 [%s] 成功。', 'goodslib'), $attr['attr_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_spec_attribute/edit', array('attr_id' => $attr_id))));
		} else {
	...
	public function edit() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('编辑属性', 'goodslib')));
		$this->assign('ur_here', __('编辑属性', 'goodslib'));
	...
	public function update() {
		...
		return $this->showmessage(sprintf(__('编辑规格属性 [%s] 成功。', 'goodslib'), $attr['attr_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_spec_attribute/edit', array('attr_id' => $attr_id))));
	}
	...
	public function batch() {
		...
		if (isset($_POST['checkboxes'])) {
			...
			return $this->showmessage(sprintf(__('成功删除了 %d 条商品属性', 'goodslib'), $count), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_spec_attribute/init', array('cat_id' => $cat_id))));
		}
	}
	...
	public function edit_attr_name() {
		...
		if (!empty($val)) {
			...
			if (RC_DB::table('attribute')->where('attr_id', $id)->update($data)) {
				...
				return $this->showmessage(__('编辑属性名称成功', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('content' => $val));
			}
		} else {
