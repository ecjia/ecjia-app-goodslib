		...
		<form class="form-horizontal" action="{$form_action}" method="post" name="theForm">
			<fieldset>
				<div class="control-group formSep">
					<label class="control-label">{t domain="goodslib"}参数模板名称：{/t}</label>
					<div class="controls">
				...
				<div class="control-group formSep">
					...
					<div class="controls chk_radio">
						<input class="uni_style" type="radio" name="enabled" value="1" {if $parameter_template_info.enabled eq 1} checked="checked" {/if}/><span>{t domain="goodslib"}启用{/t}</span>
						<input class="uni_style" type="radio" name="enabled" value="0" {if $parameter_template_info.enabled eq 0} checked="checked" {/if}/><span>{t domain="goodslib"}禁用{/t}</span>
					</div>
				</div>
				...
				<div class="control-group formSep">
					...
					<div class="controls">
						...
						<span class="help-block">{t domain="goodslib"}多个参数组时，换行输入，排序也将按照自然顺序排序。{/t}</span>
					</div>
				</div>
