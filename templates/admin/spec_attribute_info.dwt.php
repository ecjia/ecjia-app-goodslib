	...
	<div class="span12">
		<form class="form-horizontal" name="theForm" action="{$form_action}" method="post">
			<fieldset>
				...
				<div class="control-group formSep">
					<label class="control-label">{t domain="goodslib"}所属规格模板：{/t}</label>
				...
				<div class="control-group formSep">
					<label class="control-label">{t domain="goodslib"}属性类型：{/t}</label>
					<div class="controls chk_radio">
						<input class="uni_style" type="radio" name="attr_cat_type" value="0" {if $attr.attr_cat_type eq 0} checked="checked" {/if}/><span>{t domain="goodslib"}普通{/t}</span>
						<input class="uni_style" type="radio" name="attr_cat_type" value="1" {if $attr.attr_cat_type eq 1} checked="checked" {/if}/><span>{t domain="goodslib"}颜色{/t}</span>
					</div>
				</div>
