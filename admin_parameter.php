...
class admin_parameter extends ecjia_admin {
	public function __construct() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('商品库参数模板', 'goodslib'), RC_Uri::url('goodslib/admin_parameter/init')));
	}
	...
	public function init() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('商品库参数模板', 'goodslib')));
		...
		$this->assign('form_search',  		RC_Uri::url('goodslib/admin_parameter/init'));
	...
	public function add() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('添加参数模板', 'goodslib')));
		$this->assign('ur_here', __('添加参数模板', 'goodslib'));
		$this->assign('action_link', array('href'=>	RC_Uri::url('goodslib/admin_parameter/init'), 'text' => __('参数模板列表', 'goodslib')));
		
		$this->assign('action', 'add');
		
		$this->assign('parameter_template_info', array('enabled' => 1));
		
		$this->assign('form_action',  RC_Uri::url('goodslib/admin_parameter/insert'));
	...
	public function insert() {
		...
		if ($count > 0 ){
			return $this->showmessage(__('参数模板名称已存在。', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		} else {
			$cat_id = RC_DB::table('goods_type')->insertGetId($par_template);
			if ($cat_id) {
				return $this->showmessage(__('添加参数模板成功', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_parameter/edit', array('cat_id' => $cat_id))));
			} else {
				return $this->showmessage(__('添加参数模板失败', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
		}
	}
	...
	public function edit() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('编辑参数模板', 'goodslib')));
		$this->assign('ur_here', __('编辑参数模板', 'goodslib'));
		$this->assign('action_link', array('href'=>RC_Uri::url('goodslib/admin_parameter/init'), 'text' => __('参数模板列表', 'goodslib')));
		...
		$this->assign('form_action', RC_Uri::url('goodslib/admin_parameter/update'));
	...
	public function update() {
		...
		if ($count > 0) {
			...
			return $this->showmessage(__('编辑参数模板成功。', 'goodslib'),ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_parameter/edit', array('cat_id' => $cat_id))));
		}
	}
